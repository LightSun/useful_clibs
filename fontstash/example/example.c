...
int main()
{
    ...
    fontNormal = fonsAddFont(fs, "sans", "../fontstash/example/DroidSerif-Regular.ttf");
	if (fontNormal == FONS_INVALID) {
		printf("Could not add font normal.\n");
		return -1;
	}
    fontItalic = fonsAddFont(fs, "sans-italic", "../fontstash/example/DroidSerif-Italic.ttf");
	if (fontItalic == FONS_INVALID) {
		printf("Could not add font italic.\n");
		return -1;
	}
    fontBold = fonsAddFont(fs, "sans-bold", "../fontstash/example/DroidSerif-Bold.ttf");
	if (fontBold == FONS_INVALID) {
		printf("Could not add font bold.\n");
		return -1;
	}
    fontJapanese = fonsAddFont(fs, "sans-jp", "../fontstash/example/DroidSansJapanese.ttf");
	...
	while (!glfwWindowShouldClose(window))
	{
        ...
        fonsDrawText(fs, dx,dy,"中国方法私はガラスを食べられます。それは私を傷つけません。",NULL);
